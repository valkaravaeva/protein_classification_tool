#!/usr/bin/env python3
"""
Normalize the number of uncharacterized proteins per genome by that genome's total
CDS count, then compute median/mean/max/min normalized values per taxonomic rank
(phylum, class, order, family, genus, species, strain, genome).

Nextflow process: countUnch
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Normalize the number of uncharacterized proteins per genome by that genome's total CDS count, then compute median/mean/max/min normalized values per taxonomic rank (phylum, class, order, family, genus, species, strain, genome)."
    )
    parser.add_argument("dbfile", help="path to taxonomy file")
    parser.add_argument("save_unch", help="path to uncharacterized file")

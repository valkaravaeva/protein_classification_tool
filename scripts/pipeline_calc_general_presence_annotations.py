#!/usr/bin/env python3
"""
Build a per-accession presence/absence matrix across annotation databases (KO, KEGG
pathway, InterPro, Pfam, CDD, ...), then summarize how often each annotation source
(and each combination of sources) occurs across all accessions.

Nextflow process: summaryAnno
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Build a per-accession presence/absence matrix across annotation databases (KO, KEGG pathway, InterPro, Pfam, CDD, ...), then summarize how often each annotation source (and each combination of sources) occurs across all accessions.'
    )
    parser.add_argument("annofile", help="path to file from 01_archaea35k_merge_all_annotations.py")
    parser.add_argument("savefile_matrix", help="where to save presence absence matrix output to")
    parser.add_argument("savefile_counts", help="path to save the file with counts")
    parser.add_argument("summary_save_counts", help="path to save the file with the summary of the counts")
...
def main() -> None:
    ...
    savefile_matrix = args.savefile_matrix
    savefile_counts = args.savefile_counts
    summary_save_counts = args.summary_save_counts

#!/usr/bin/env python3
"""
Calculate KEGG module completeness per genome, accounting for both KO complexes
(e.g. K00001+K00002) and alternative KOs (e.g. K00001,K00002). Runs in four stages:
  1. Presence of each complex per genome.
  2. Presence of each group of alternative KOs per genome.
  3. Combined module completeness (fraction of steps present) per genome.
  4. Binning genomes into completeness categories (0%%, <50%%, <75%%, >=75%%, 100%%)
     per module, with a taxonomic (class-level) breakdown.

Nextflow process: calcModule
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Calculate KEGG module completeness per genome, accounting for both KO complexes (e.g. K00001+K00002) and alternative KOs (e.g. K00001,K00002). Runs in four stages:   1. Presence of each complex per genome.   2. Presence of each group of alternative KOs per genome.   3. Combined module completeness (fraction of steps present) per genome.   4. Binning genomes into completeness categories (0%%, <50%%, <75%%, >=75%%, 100%%)      per module, with a taxonomic (class-level) breakdown.'
    )
    parser.add_argument("modulesfile", help="kegg module architecture file")
    parser.add_argument("allgenfile", help="accgen file")
    parser.add_argument("absentmodfile", help="no hit modules txt file")
    parser.add_argument("cutoffs", help="ko_list file")
    parser.add_argument("taxonomy", help="taxonomy")
    parser.add_argument("modulenamefile", help="module fullness file from previous step")
    parser.add_argument("complexpresence", help="complex_presence_per_genome.tsv")
    parser.add_argument("altpresence", help="alternative_presence_per_genome.tsv")
    parser.add_argument("save_pergenome", help="modules_presence_per_genome.tsv")
    parser.add_argument("savefile_perc", help="module_completeness_percent.tsv")
    parser.add_argument("moddir", help="per module directory")
...
def main() -> None:
    ...
    allgenfile = args.allgenfile
    absentmodfile = args.absentmodfile
    cutoffs = args.cutoffs
    taxonomy = args.taxonomy
    modulenamefile = args.modulenamefile
    complexpresence = args.complexpresence
    altpresence = args.altpresence
    save_pergenome = args.save_pergenome
    savefile_perc = args.savefile_perc
    moddir = args.moddir
    ...
    # FIXED: this whole block (M00617 fix + writing save_pergenome) was duplicated in the
    # original script - a second, identical copy ran immediately after this one but recomputed
    # the M00617 sum WITHOUT the "cap at 1" step below, then overwrote save_pergenome again, so
    # the capped value computed here never actually reached the output file. The duplicate
    # (uncapped) copy has been removed; this capped version is now the one that's saved.

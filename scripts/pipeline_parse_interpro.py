#!/usr/bin/env python3
"""
Parse raw InterProScan TSV output for a list of genomes into one combined
per-accession table with one column per member database (Pfam, CDD, PANTHER, ...).

Nextflow process: parseInterpro
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Parse raw InterProScan TSV output for a list of genomes into one combined per-accession table with one column per member database (Pfam, CDD, PANTHER, ...).'
    )
    parser.add_argument("genomefile", help="input list of genomes")
    parser.add_argument("savefile", help="output name")

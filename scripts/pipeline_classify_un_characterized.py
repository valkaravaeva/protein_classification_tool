#!/usr/bin/env python3
"""
Classify every annotated protein accession as "characterized" or "uncharacterized",
based on its KO/KEGG-pathway, PANTHER, TIGRFAM and Hamap annotations and curated
exclusion lists. Accessions with no annotation at all are also captured as
uncharacterized.

Nextflow process: classifyUnch
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Classify every annotated protein accession as "characterized" or "uncharacterized", based on its KO/KEGG-pathway, PANTHER, TIGRFAM and Hamap annotations and curated exclusion lists. Accessions with no annotation at all are also captured as uncharacterized.'
    )
    parser.add_argument("fulltable", help="path to the full table containing all annotations")
    parser.add_argument("unchtigrs", help="path to the manually curated list of uncharacterized TIGRFAMs")
    parser.add_argument("pthrexclfile", help="path to the manually curated list of uncharacterized PANTHERS")
    parser.add_argument("accgentable", help="path to a file that contains mappings of accessions to the genomes in a tabular format")
    parser.add_argument("dbfile", help="path to the genome database file")
    parser.add_argument("save_unch", help="path to save uncharacterized")
    parser.add_argument("save_ch", help="path to save characterized")
    parser.add_argument("save_summary", help="path to save summary counts")
...
def main() -> None:
    ...
    unchtigrs = args.unchtigrs
    pthrexclfile = args.pthrexclfile
    accgentable = args.accgentable
    dbfile = args.dbfile
    save_unch = args.save_unch
    save_ch = args.save_ch

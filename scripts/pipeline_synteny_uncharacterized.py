#!/usr/bin/env python3
"""
Run a (non-continuous) synteny analysis centered on uncharacterized-protein hits:
build a merged feature table from NCBI feature tables / GFFs, index genes by
position on each chromosome/contig, extract the +/-2 gene neighborhood of every
uncharacterized hit, and count how often particular neighborhood "blocks" recur
across genomes. Excludes genomes that were later filtered out by quality.

Nextflow process: syntenyUnch
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Run a (non-continuous) synteny analysis centered on uncharacterized-protein hits: build a merged feature table from NCBI feature tables / GFFs, index genes by position on each chromosome/contig, extract the +/-2 gene neighborhood of every uncharacterized hit, and count how often particular neighborhood "blocks" recur across genomes. Excludes genomes that were later filtered out by quality.'
    )
    parser.add_argument("taxfile", help="taxonomy file")
    parser.add_argument("unchfile", help="files with uncharacterized hits")
    parser.add_argument("fulltable", help="full table")
    parser.add_argument("savefile_merged", help="'synteny_merged_table.tsv'")
    parser.add_argument("savefile_indexed", help="'synteny_indexed_table.tsv'")
    parser.add_argument("savefile_neighbors", help="'synteny_neighbors.tsv'")
    parser.add_argument("savefile_count_blocks", help="'synteny_count_uncharacterized_blocks.tsv'")
    parser.add_argument("save_counts_overten", help="'synteny_count_uncharacterized_blocks_more_than_ten.tsv'")
    parser.add_argument("save_counts_per_genome", help="'synteny_count_uncharacterized_blocks_per_genome.tsv'")
...
def main() -> None:
    ...
    unchfile = args.unchfile
    fulltable = args.fulltable
    savefile_merged = args.savefile_merged
    savefile_indexed = args.savefile_indexed
    savefile_neighbors = args.savefile_neighbors
    savefile_count_blocks = args.savefile_count_blocks
    save_counts_overten = args.save_counts_overten
    save_counts_per_genome = args.save_counts_per_genome

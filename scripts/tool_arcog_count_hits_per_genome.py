#!/usr/bin/env python3
"""
Count arCOG hits per genome (total hits, and "classified" hits that exclude
poorly-characterized/function-unknown functional categories), and summarize the
percentage of unclassified sequences per phylum.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Count arCOG hits per genome (total hits, and "classified" hits that exclude poorly-characterized/function-unknown functional categories), and summarize the percentage of unclassified sequences per phylum.'
    )
    parser.add_argument("arcogfile", help="arcog map file")
    parser.add_argument("taxfile", help="taxonomy.tsv")
    parser.add_argument("savefile", help="'arcog_hits_counts_per_genome.tsv'")
    parser.add_argument("savecounts", help="'arcog_hits_mean_median_per_phylum.tsv'")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    arcogfile = args.arcogfile
